...
export const addListItem = (state, action, listName) => {
  ...
  return {
    ...
    [listName]: [...resultantList, ...state[listName].filter(l => l.delete)]
  };
};
...
export const removeListItem = (state, action, listName) => {
  ...
  newItems = itemToRemove.entityId
    ? [
      ...
      ...resultantItems, ...state[listName].filter(l => l.delete)
      ]
    : [...state[listName].slice(0, index - 1), ...resultantItems, ...state[listName].filter(l => l.delete)];
