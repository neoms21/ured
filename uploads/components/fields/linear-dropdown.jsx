...
class LinearDropdown extends Component {
  ...
  componentDidMount() {
    if (this.state.selectedItem)
      this.props.input.onChange(this.state.selectedItem.id);
  }

  componentWillReceiveProps(nextProps) {
    if (nextProps.selectedItem !== this.props.selectedItem) {
      this.setState({
        selectedItem: nextProps.items.find(i => i.id === nextProps.selectedItem)
      });
    }
  }
  ...
  selectValue = item => {
    if (this.props.input) {
      ...
      this.props.input.onBlur(item.id);
    }
  ...
  renderListItems = (items, fieldName) => {
  ...
  render() {
    ...
    return (
      <div className={` ${styles.container} `} id={name ? name : "linearList"}>
        ...
        >
          {this.renderListItems(displayItems, name)}
