...
export default function RFReactSelect({input, options, multi, classNames, labelKey, valueKey, label, helpText, errorText, meta: {touched, error}}) {
    ...
    return (
        <div className={classNames} id={name}>
           ...
           /></div>
            {(touched && error && !error[name] && <div className={`form-error`}>{errorText}</div>)}
        </div>
    );
}
