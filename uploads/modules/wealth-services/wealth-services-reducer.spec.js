...
describe("Wealth services Reducer", () => {
  ...
  it("should map the objectives to other if not present in list", () => {
    ...
    expect(selectedObjectives).toEqual([
      ...
      { id: "2", value: "Growth of capital", selected: true },
      { id: "4", value: "Retirement", selected: true },
      { id: "other", value: "abcd, 1234", selected: true }
    ]);
  });
  ...
  it("should clear the other objective", () => {
    const state = {
      fields: {
        a: { value: "aa" },
        b: { value: "bb" },
        c: { value: "cc" },
        otherWealthPlanningObjective: { value: "ooo" }
      }
    };

    const result = reducer(state, { type: CLEAR_OTHER_WEALTH_OBJECTIVE });

    expect(result.fields).toEqual({
      a: { value: "aa" },
      b: { value: "bb" },
      c: { value: "cc" },
      otherWealthPlanningObjective: { value: undefined, deleted: true }
    });
  });
});
