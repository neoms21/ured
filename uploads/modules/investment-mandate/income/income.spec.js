...
import { ADD_REGULAR_PAYMENT } from "./income-action-types";
...
describe("portfolio income unit tests", () => {
  ...
  it("should not make the initial calls if data is already laoded", () => {
    ...
    expect(proxyStub.getCalls().length).toEqual(1);
  });
  ...
  it("should load the page and fire the bank accounts requests if not loaded", () => {
    ...
    expect(args).toEqual(["portfolio-income", "bank-accounts-1"]);
  });
  ...
  it("should display the regular payments", () => {
    ...
    sinon.assert.calledWith(dispatchSpy, {
      ...
      payload: { currentValues: {
        portfolioIncomeRequired: undefined,
        portfolioIncomeSeparate: true,
        portfolioManageIncome: 2,
        portfolioManageIncomeBankAccountId: undefined,
        regularPaymentAmount1: "2,034",
        regularPaymentBankAccountId1: "22",
        regularPaymentFrequency1: "monthly",
        regularPaymentSource1: 1,
        regularPaymentStartDate1: "30/06/2018"
      }  }
    });
  });
