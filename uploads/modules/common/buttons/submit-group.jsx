...
class SubmitGroup extends Component {
  ...
  savePartial = () => {
    const keys = Object.keys(this.props.errors);
    let fieldsWithFailedValidations = [];
    keys.forEach(k => {
      if (typeof this.props.errors[k] === "string") {
        fieldsWithFailedValidations.push(k);
      }
    });

    this.props.savePartial(fieldsWithFailedValidations);
  };
