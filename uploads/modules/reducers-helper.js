  ...
  if (state.entities) {
    state.entities.forEach(e => {
      e.fields.forEach(fieldName => {
        // if (e.prefix) fieldName = `${e.prefix}${fieldName}`;

        if (e.prefix) {
          const prefixedName = `${e.prefix}${fieldName}`;
          objFields[prefixedName] = {
            ...schema[fieldName],
...
export const addListItem = (state, action, listName, nonListValues) => {
  let resultantList = [];
  const { schema } = state;
  ...
  let allFields = { ...resultantList.fields };

  if (state.nonListFields)
    state.nonListFields.forEach(f => {
      allFields = {
        ...allFields,
        [f]: {
          ...schema[f],
          value:
            nonListValues && nonListValues[f] !== undefined
              ? nonListValues[f]
              : currentValues[f],
          key: f
        }
...
export const removeListItem = (state, action, listName, nonListValues) => {
  let resultantItems = [];
  ...
  if (state.nonListFields)
    state.nonListFields.forEach(f => {
      allFields = {
        ...
        [f]: {
          ...state.schema[f],
          value: nonListValues ? nonListValues[f] : currentValues[f],
          key: f
        }
      };
    });
...
export const selectObjectives = (state, objectiveString) => {
  if (!objectiveString)
    return [...state.objectives.map(o => ({ ...o, selected: false }))];
  let newObjectives = [];
  const returnedObjectives = objectiveString.split(",").map(x => x.trim());
