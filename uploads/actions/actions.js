...
export function saveListFields(
  ...
  return dispatch => {
    ...
    return httpProxy
      .write(nullifyUndefinedFields(reqData, page))
      .then(response => {
...
function nullifyUndefinedFields(requestData, page) {
  let newObj = { data: {}, page: page };
  Object.keys(requestData).forEach(f => {
    newObj.data[f] = requestData[f] === undefined ? null : requestData[f];
  });
